    ...
    public static class AutoPaths {
        ...
        public AutoPaths(Follower follower) {
            ...
            barrierToCollecting = follower.pathBuilder()
                    .addPath(new BezierLine(
                            new Pose(15.89797507788162, 60.27554517133957),
                            new Pose(9.5, 42.60825545171338)
                    ))
            ...
            cycleBarrierToShoot = follower.pathBuilder()
                    .addPath(new BezierCurve(
                            new Pose(9.274143302180683, 42.51401869158879),
                            new Pose(55.542, 57.526),
                            new Pose(55.400, 82.500)
                    ))
                    .setLinearHeadingInterpolation(Math.toRadians(90), Math.toRadians(137.0))
                    .build();
        }
    }
    ...
    public void start() {
        schedule(
                sequential(
                        ...
                        Groups.loop(
                                sequential(
                                        followWithTimeout(follower, paths.cycleShootToSecondLineApproach, DEFAULT_PATH_TIMEOUT_MS),
                                        followWithTimeout(follower, paths.cycleLineToBarrier, CYCLE_BARRIER_TIMEOUT_MS),
                                        collectLine(paths.barrierToCollecting, BARRIER_INTAKE_POWER, CYCLE_COLLECT_TIMEOUT_MS),
                                        followWithTimeout(follower, paths.cycleBarrierToShoot, DEFAULT_PATH_TIMEOUT_MS),
                                        shootThreeBalls()
                                )
                        )
                )
